...
immgen_ref <- fetchReference("immgen", "2024-02-26")
immgen_ref
```
...
immgen_ref = celldex.fetch_reference("immgen", "2024-02-26")
print(immgen_ref)
```
...
cell_labels <- SingleR(test = sce, ref = immgen_ref, labels = immgen_ref$label.main)
...
# convert counts to scipy sparse
mat = sce.assays["counts"]
import delayedarray as da
mat_csc = da.to_scipy_sparse_matrix(mat, "csc")
log_normed_sce = scranpy.log_norm_counts(mat_csc, options=options)

import celldex
immgen_ref = celldex.fetch_reference("immgen", "2024-02-26")
log_ref_csc = da.to_dense_array(immgen_ref.assays["logcounts"])

matches = singler.annotate_single(
    test_data=log_normed_sce, 
    test_features=sce.get_row_names(), 
    ref_data = log_ref_csc,
