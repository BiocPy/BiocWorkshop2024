...
```bash
pip install -U biocutils genomicranges rds2py numpy pandas
```
...
Let's download the human reference genome and save the exon positions group by transcripts.
...
:::{.callout-note}
While gene id's are unique, gene symbols are not. In addition, this list has genes with no symbols.
:::
...
Let's use the `range()` method to the get the full extent of each gene.
...
Then resizing to a width of 1 base pair at the start of each range to pinpoint the TSS.
...
:::{.callout-note}
Please be aware that because gene symbols may not be unique, this GenomicRanges object might contain duplicates. One might want to resolve duplicate symbols by making the symbols unique. We will leave this as an exercise to the reader.
:::
...
For the purpose of this tutorial, lets download a bed file containing peaks from a ChIP-seq experiments on "Human B cells" to identify "EZH2" binding sites (from ENCODE) and cataloged in [bedbase.org](https://bedbase.org/bed/be4054acf6e3feeb4dc490e6430e358e).
...
:::{.callout-note}
This yields same results for this particular scenario, but may not if the 'peaks' contain stand information.
:::
...
Let's first identify intro regions. We will use the `by_gene` object we created that contains a `GenomicRangesList` split by gene.
...
:::{.callout-note}
These add up to over 100%, because some peaks overlap both introns and exons depending on how wide the peaks are. Ideally you may want to filter the peaks based on preference as you annotate them with TSS, promoters etc.
:::
...
:::{.callout-note}
- This is by transcript.
- Not really sure what the rationale would be, but we are mostly showcasing complex genomic operations that are possible with the package.
:::
...
- Shifting ranges can be used to look at regions adjacent to your features of interest. e.g. defining a crips cleavage site from the guide binding site.
